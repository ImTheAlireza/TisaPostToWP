...
**وضعیت اجرا (فاز ۹ — تست و انتشار) — انجام شد ✅**

| آیتم پلن | وضعیت | آنچه اجرا شد |
|---|---|---|
| `pytest-cov --cov-fail-under` | ✅ دو کف، اندازه‌گیری‌شده | CI دو floor دارد: `--cov=bot` ≥ **۷۵** و `--cov=bot/services` ≥ **۸۵** (عددِ امروز: ۸۱٪ و ۸۹٪). کف‌ها آرزو نیستند: هر کدام چند درصد زیرِ اندازه‌گیریِ واقعی‌اند تا با یک PRِ معمولی نشکنند، ولی اگر کسی تستی را پاک کند، CI قرمز می‌شود. `tests/test_release.py` چک می‌کند workflow همان دو عدد را داشته باشد — قولِ پلن با تنظیماتِ CI مقایسه می‌شود، نه با حرف. |
| تست خواصی (hypothesis) | ✅ ۱۴ خاصیت | `tests/test_properties.py`: چهار دستهٔ invariant که دقیقاً همان گره‌های بحرانی‌اند — `plan.count ≤ naive` و هیچ محدودیتِ رنگی مدلی را از جریان حذف نمی‌کند؛ مبلغِ دارایِ واحدِ صریح دوباره مقیاس نمی‌خورد و عددِ پشتِ «وزن/تاریخ/کد…» قیمت نمی‌شود؛ قیمت بیرونِ بازه **همیشه** گزارش می‌شود (نه پذیرشِ بی‌صدا، نه ردِ بی‌صدا)؛ و دو خطِ مدلِ متفاوت هرگز یک لیبل نمی‌گیرند. `hypothesis` در `requirements.txt` نیست (ریسکِ استقرار روی هاست مشترک) و فایل با `skipUnless` خودش را خاموش می‌کند. |
| corpus استخراج | ✅ ۸ پست × ۱۱ فیلد | `tests/test_extraction_corpus.py`: ورودی = پیامِ **کاملِ** فروشنده (کپشن + PRODUCT INFO با هم، همان‌طور که ربات می‌گیرد)، خروجی = هر خانه‌ای که به ووکامرس می‌رود؛ یک خانه جابه‌جا شود، تست می‌گوید کدام محصول. همین corpus سه باگ را بیرون داد و هر سه در `product_extractor`/`color_matrix` اصلاح شد: (۱) «قیمت ویژه ۴۲۰٬۰۰۰» قیمتِ اصلی را می‌بلید (حالا هر دو عدد درست جا می‌افتند)، (۲) «ناموجود» نامِ محصول می‌شد، (۳) رنگِ مدلِ نوشتاریِ فارسی («۱۳ پرو مکس: مشکی، سفید») به `iPhone 13 Pro` می‌چسبید و واریژن‌ها را غلط می‌بست — با تاواوردنِ واژه‌های فارسیِ همان `fold_variant_words` که `_parse_single_iphone` به‌کار می‌برد (دو پیاده‌سازی نشد). |
| تست قرارداد با `docker-compose.yml` (WP + WooCommerce + MySQL) | 🟡 نوشته شد، **اجرا نشده** | `docker-compose.yml` + `deploy/bootstrap-wordpress.sh` + `tests/test_contract_wordpress.py` + `docs/CONTRACT-TESTS.md` + `.github/workflows/contract.yml` (`workflow_dispatch`، نه در `pull_request`). پوششِ قرارداد: بارکدِ تکراری → 409 و پیامِ خالی برای فایلِ پاک‌شده؛ «۰» موجودی و نه «−»؛ `menu_order` صفر-مبنا؛ ردیفِ بی‌SKU رد می‌شود بقیه نه؛ و **راندِ دومِ همان فایل** هیچ محصولِ تکراری نسازد. در این محیط Docker نداریم، پس هیچ‌کدام از این پنج سناریو اجرا **نشده** — ادعای سبز بودن ندارند؛ `TestContractWooRuns` تا `TISA_CONTRACT=1` خودش را skip می‌کند. |
| SemVer + `CHANGELOG.md` + release GitHub؛ «افزونه از source build شود» | ✅ با یک انحرافِ آگاهانه | `docs/RELEASING.md` (قاعدهٔ ۰.x، هفت گام، و «چه چیزی در ریلیز نمی‌آید»). بیلد افزونه: `scripts/build_plugin_zip.py` — **قطعی** (`ZipInfo` با stamp ثابت، ترتیب و external_attr قفل‌شده، `allowZip64=False`) و `--check` در CI. انحراف: زیپ هنوز ترک‌شده است، چون روی هاستِ `git pull` ابزار بیلد نیست؛ همان تضمینِ «از سورس ساخته شود» را `--check` می‌دهد (زیپ = بیلدِ تازهٔ سورس، وگرنه CI قرمز) + `tests/test_release.py` که با یک درختِ تمپ اثبات می‌کند `--check` اختلاف را **واقعاً** می‌گیرد. `Version:` سرِ فایل PHP با نسخهٔ زیپ و `read_version()` یکی است؛ ریلیز یادآوری می‌کند اگر قرارداد عوض شده، بالا برود. |
| DoD برای هر PR | ✅ | `.github/pull_request_template.md`: بندِ اولش همان DoD پلن است (تست رگرسیون که **پیش از** اصلاح شکست می‌خورد، README، CHANGELOG، اجرای corpus برای تغییرِ مسیر استخراج) + شش بندی که در فازهای ۰ تا ۸ واقعاً نقض شده بودند (کارتی که عددش را نفرستد، دکمه‌ای که هندلر نداشته باشد، promise در docstring بدون صفحه، …). |

**معیار پذیرش فاز ۹:** `pytest -q` با دو کفِ پوشش سبز باشد و `ruff check bot tests main.py scripts`
صفر؛ هر چهار خاصیت روی ورودی تصادفی بچرخند و هر هشت پستِ corpus فیلدبه‌فیلد پاس شوند؛
`python scripts/build_plugin_zip.py --check` در CI سبز باشد و با یک ویرایشِ PHP قرمز؛
`tests/test_release.py` نسخه، زیپ، compose، کف‌های CI و مسیرهای مستندات را به‌صورت
متقابل بسنجد (مسیری که README/compose نام می‌برند باید وجود داشته باشد)؛ تست قرارداد
نوشته باشد و صادقانه «اجرا نشده» بگوید؛ و چک‌لیستِ تستِ دستیِ فازهای ۰ تا ۹ یک‌جا
به مالک داده شود — `docs/MANUAL-TEST-CHECKLIST.md` (۱۴ بخش، از `.env` تا ریلیز).
