...
| بازپخشِ اثر («روی ۷ محصول آخر، ۲۳ واریژن را کم می‌کرد») | ✅ | `learning_corpus.py` (حلقهٔ ۲۰ استخراجِ آخر در `data/learning_corpus.json`) + `learning_impact.py`. عددِ واریژن با `color_matrix.variation_count`ِ واقعی حساب می‌شود و واژه‌ها با `learning.replace_term` — یعنی همان توابعی که موقع ساخت مصرف می‌شوند؛ یک «پیش‌بین»ِ دوم که ممکن است با واقعیت اختلاف پیدا کند نداشتیم |
...
۵۷ تست تازه (مجموع ۵۷۹؛ ruff/mypy/coverage سبز).
